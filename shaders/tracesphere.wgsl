...
fn lineSphereIntersection(s: vec3f, d: vec3f, c: vec3f, radius: f32) -> vec4f {
    // Normalize direction vector d
    var lengthD = sqrt(d[0] * d[0] + d[1] * d[1] + d[2] * d[2]);
    var dNorm = vec3f(d[0] / lengthD, d[1] / lengthD, d[2] / lengthD);
    ...
    var q = vec3f(s[0] - c[0], s[1] - c[1], s[2] - c[2]);
    // var q = s - c;
    //The line can be described as p = s + dt
    //want to find some ||p - c|| = ||r||, where ||x|| is the magnitude of the vector x
    //plug in p to get ||s + dt - c|| = ||r||

    //let q = s - c, want ||q + dt|| = ||r||
    //q^2 + 2qdt + (dt)^2 = r^2          d^2t^2 + t2qd + q^2 - r^2
    //Quatratic of the form A = d^2 = 1 since d is normalized, B = 2qd which we shall take to be the dot product, c = q^2 - r^2 which we shall take to be the square magnitudes
    var A = 1.0;
    var B = 2 * (q[0] * dNorm[0] + q[1] * dNorm[1] + q[2] * dNorm[2]); // 2 * dot(q, d)
    var C = q[0] * q[0] + q[1] * q[1] + q[2] * q[2] - radius * radius; 
    ...
    if (discriminant < 0) {
        ...
        if (t1 >= 0) {
            return vec4f(t1, s[0] + t1 * dx, s[1] + t1 * dy, s[2] + t1 * dz);
        }
        var t2 = (-B + sqrt(discriminant)) / (2.0 * A);
        if (t2 >= 0) {
            return vec4f(t2, s[0] + t2 * dx, s[1] + t2 * dy, s[2] + t2 * dz);
        }
...
fn assignSphereColor(uv: vec2i, hit: f32, spherePoint: vec3f) {
  var color: vec4f;
  if (hit > 0) { // hit 
    var point = vec4f(spherePoint.xyz, 1);
    var vector = point - sphere.center;
    var magnitude = sqrt(dot(vector, vector));
    color = vector / magnitude;
  }
  else { // no hit
...
fn assignBoxColor(uv: vec2i, t: f32, idx: i32) {
  var color: vec4f;
  if (t > 0) { // hit 
    switch(idx) {
      case 0: { //front
...
fn assignColor(uv: vec2i, boxT: f32, idx: i32, sphereT: f32, spherePoint: vec3f) {
   if (0 < boxT && 0 < sphereT){
      if (sphereT <= boxT) {
        assignSphereColor(uv, sphereT, spherePoint);
      }
      else {
        assignBoxColor(uv, boxT, idx);
      }
   }
   else {
    if (0 < boxT) {
...
fn computeOrthogonalMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    var sphereHit = lineSphereIntersection(spt, rdir, sphere.center.xyz, sphere.radius);
    var point = vec3f(sphereHit.x, sphereHit.y, sphereHit.z);
    var boxHit = rayBoxIntersection(spt, rdir);
    var idx = i32(boxHit[0]);
    // assign colors
    assignColor(uv, boxHit[0], idx, sphereHit[0], point);
  }
}
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    // compute the intersection to the object
    var sphereHit = lineSphereIntersection(spt, rdir, sphere.center.xyz, sphere.radius);
    var point = vec3f(sphereHit[1], sphereHit[2], sphereHit[3]);
    var boxHit = rayBoxIntersection(spt, rdir);
    var idx = i32(boxHit[1]);
    // assign colors
    assignColor(uv, boxHit[0], idx, sphereHit[0], point);
  }
}

fn normalizeVector(vector : vec3f) -> vec3f {
    var magnitude = sqrt(vector[0] * vector[0] +vector[1] * vector[1] + vector[2] * vector[2]);
    return vec3f(vector[0] / magnitude, vector[1] / magnitude, vector[2] / magnitude);
  }