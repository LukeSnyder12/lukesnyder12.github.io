...
fn getPointLight(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo {
...
fn getDirectionalLight(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo{
  //intensity is always just the intensity of the light source
  var out : LightInfo;
...
fn getSpotLight(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo{
  var out: LightInfo;
  var cutAngle = light.params[1];
  var viewDirection = normalize(hitPoint - lightPos);
  var dotProd = dot(normalize(lightDir), viewDirection);
  if (dotProd < cos(cutAngle)){
    out.intensity = vec4f(0, 0, 0, 1);
    out.lightdir = light.direction.xyz;
    return out;
  }
  var numerator = light.intensity * pow(dotProd, light.params[2]);
  var temp = hitPoint - lightPos;
  var distance = dot(temp, temp);
  var denominator = light.attenuation[0] + light.attenuation[1] * distance + light.attenuation[2] * distance * distance;
...
fn getPhongLightInfo(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo{
  if (light.params[0] == 0){
    return getPointLight(lightPos, lightDir, hitPoint, objectNormal);
  }
  else if (light.params[0] == 1){
    return getDirectionalLight(lightPos, lightDir, hitPoint, objectNormal);
  }
  else{
    return getSpotLight(lightPos, lightDir, hitPoint, objectNormal);
  }
}

fn getLambertLightInfo(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo{
  var viewDirection = normalize(hitPoint - lightPos);
  if (light.params[0] == 0){
    var result = getPointLight(lightPos, lightDir, hitPoint, objectNormal);
    result.intensity *= max(dot(viewDirection, -objectNormal), 0);
    return result;
  }
  else if (light.params[0] == 1){
    var result = getDirectionalLight(lightPos, lightDir, hitPoint, objectNormal);
    result.intensity *=  dot(normalize(lightDir), viewDirection);
    return result;
  }
  else{
    var result = getSpotLight(lightPos, lightDir, hitPoint, objectNormal);
    result.intensity *= max(dot(viewDirection, -objectNormal), 0);
    return result;
  }
...
fn computeOrthogonalMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    var setting = light.params[3];
    if (setting == 0){
      lambertShade(uv, spt, rdir);
    }
    else if (setting == 1){
      phongShade(uv, spt, rdir);
    }
    else{
      toonShade(uv, spt, rdir);
    }
  }
}
...
fn toonShade(uv: vec2i, spt: vec3f, rdir: vec3f){
  // orthogonal camera ray sent from each pixel center at z = 0
    var hitInfo = rayBoxIntersection(spt, rdir);
    // assign colors
    ...
    if (hitInfo.x > 0) { // if there is a hit
      ...
      var lightPos = applyMotorToPoint(light.position.xyz, reverse(cameraPose.motor));
      var lightDir = applyMotorToDir(light.direction.xyz, reverse(cameraPose.motor));
      lightDir = applyMotorToDir(lightDir, reverse(box.motor));
      lightDir /= box.scale.xyz;
      lightDir = normalize(lightDir);
      lightPos = applyMotorToPoint(lightPos, reverse(box.motor));
      lightPos /= box.scale.xyz;
      ...
      let result = getPhongLightInfo(lightPos, lightDir, hitPt, normal);

      var reflect = reflect(normal, lightDir);
      var ks = vec4f(1, 1, 1, 1);
      var ke = boxEmitColor();
      var kd = boxDiffuseColor(i32(hitInfo.y));
      var gamma = 50.;
      var specular = ks * pow(max(0, dot(rdir, -reflect)), gamma);
      var ka = vec4f(.1, .1, .1, .1);
      color =  (ke * result.intensity + kd * max(0, dot(normal, -result.lightdir)) * result.intensity + specular * result.intensity + ka * result.intensity);
      color = round(color * 4.) / 4.;
      color[3] = 1;
    }
...
fn phongShade(uv: vec2i, spt: vec3f, rdir: vec3f){
  // orthogonal camera ray sent from each pixel center at z = 0
    var hitInfo = rayBoxIntersection(spt, rdir);
    // assign colors
    var color = vec4f(0.f/255, 56.f/255, 101.f/255, 1.); // Bucknell Blue
    if (hitInfo.x > 0) { // if there is a hit
      var normal = boxNormal(i32(hitInfo.y));
      normal = normalize(transformNormal(normal));
      //   4. transform the light to the world coordinates
      //   Note: My light is stationary, so Icancel the camera movement to keep it stationary
      var lightPos = applyMotorToPoint(light.position.xyz, reverse(cameraPose.motor));
      var lightDir = applyMotorToDir(light.direction.xyz, reverse(cameraPose.motor));
...
fn lambertShade(uv: vec2i, spt: vec3f, rdir: vec3f){
    // orthogonal camera ray sent from each pixel center at z = 0
    var hitInfo = rayBoxIntersection(spt, rdir);
    // assign colors
    var color = vec4f(0.f/255, 56.f/255, 101.f/255, 1.); // Bucknell Blue
    if (hitInfo.x > 0) { // if there is a hit
      ...
      //   6. compute the light information
      //   Note: I do the light computation in the world coordiantes because the light intensity depends on the distance and angles in the world coordiantes! If you do it in other coordinate system, make sure you transform them properly back to the world one.
      let lightInfo = getLambertLightInfo(lightPos, lightDir, hitPt, normal);
