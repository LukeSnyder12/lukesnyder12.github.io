...
export default class PerlinNoise {
  ...
  noise3d(x, y, z) {
    // Implementation ref: https://mrl.cs.nyu.edu/~perlin/noise/
    ...
    // Get hash values
    const a  = this._permutation[ix] + iy;
    const aa = this._permutation[a] + iz;
    const ab = this._permutation[a + 1] + iz;
    const b  = this._permutation[ixx] + iy;
    const ba = this._permutation[b] + iz;
    const bb = this._permutation[b + 1] + iz;
  
    // Get gradient values
    const gradAA = this.gradient(this._permutation[aa], x, y, z);
    const gradBA = this.gradient(this._permutation[ba], x - 1, y, z);
    const gradAB = this.gradient(this._permutation[ab], x, y - 1, z);
    const gradBB = this.gradient(this._permutation[bb], x - 1, y - 1, z);
    const gradAAZ = this.gradient(this._permutation[aa + 1], x, y, z - 1);
    const gradBAZ = this.gradient(this._permutation[ba + 1], x - 1, y, z - 1);
    const gradABZ = this.gradient(this._permutation[ab + 1], x, y - 1, z - 1);
    const gradBBZ = this.gradient(this._permutation[bb + 1], x - 1, y - 1, z - 1);
  
    // Trilinear interpolation
    const lerpX1 = interpolate(gradAA, gradBA, u);
    const lerpX2 = interpolate(gradAB, gradBB, u);
    const lerpX3 = interpolate(gradAAZ, gradBAZ, u);
    const lerpX4 = interpolate(gradABZ, gradBBZ, u);
  
    const lerpY1 = interpolate(lerpX1, lerpX2, v);
    const lerpY2 = interpolate(lerpX3, lerpX4, v);
  
    return interpolate(lerpY1, lerpY2, w);
  }
