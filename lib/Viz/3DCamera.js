...
export default class Camera {
  ...
  moveX(d) {
    ...
    var direction = PGA3D.applyMotorToDir([d, 0, 0], this._pose);
    var dt = PGA3D.createTranslator(direction[0], direction[1], direction[2]);
    var newpose = PGA3D.geometricProduct(dt, this._pose);
  ...
  moveY(d) {
    ...
    // Suggest to use PGA3D
    var direction = PGA3D.applyMotorToDir([0, d, 0], this._pose);
    var dt = PGA3D.createTranslator(direction[0], direction[1], direction[2]);
    var newpose = PGA3D.geometricProduct(dt, this._pose);
    this.updatePose(newpose);
  }
  ...
  moveZ(d) {
    ...
    // Suggest to use PGA3D
    var direction = PGA3D.applyMotorToDir([0, 0, d], this._pose);
    var dt = PGA3D.createTranslator(direction[0], direction[1], direction[2]);
    var newpose = PGA3D.geometricProduct(dt, this._pose);
    this.updatePose(newpose);
  }
