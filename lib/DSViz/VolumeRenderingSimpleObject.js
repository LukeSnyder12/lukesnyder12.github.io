...
export default class VolumeRenderingSimpleObject extends RayTracingObject {
  constructor(device, canvasFormat, camera, volume) {
  ...
  async createGeometry() {
    ...
    console.log("size:" + this._volume._data.length * Float32Array.BYTES_PER_ELEMENT);
    ...
    this._parameterBuffer = this._device.createBuffer({
      ...
      size: (this._volume._dims.length + 1) * Int32Array.BYTES_PER_ELEMENT,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    })

    this._device.queue.writeBuffer(this._parameterBuffer, 0, new Int32Array([this._volume._dims[0], this._volume._dims[1], this._volume._dims[2], this._setting])); //[x, y, z, march setting index]
  }

  async recreateGeometry(volume){
    this._volume = volume;
    console.log(this._volume._data);
    this._dataBuffer = this._device.createBuffer({
      label: "Data " + this.getName(),
      size: this._volume._data.length * Float32Array.BYTES_PER_ELEMENT,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    ...
    this._device.queue.writeBuffer(this._volumeBuffer, 0, new Float32Array([... this._volume._dims, 0, ...this._volume._sizes, 0]));
    this._device.queue.writeBuffer(this._dataBuffer, 0, new Float32Array(this._volume._data));
    this.createBindGroup(this._texture);
  }

  changeRenderSetting(){
    this._setting = (this._setting + 1) % 4;
    console.log("changing setting to " + this._setting);
    this._device.queue.writeBuffer(this._parameterBuffer, 0, new Int32Array([this._volume._dims[0], this._volume._dims[1], this._volume._dims[2], this._setting]));
  }
  ...
  async createShaders() {
    ...
    this._bindGroupLayout = this._device.createBindGroupLayout({
      ...
      entries: [{
        ...
        binding: 4,
        visibility: GPUShaderStage.COMPUTE,
        buffer: {  type: "read-only-storage" } // parameters for computation
      }]
    });
