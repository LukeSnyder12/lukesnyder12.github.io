...
export default class RayTracingBoxLightObject extends RayTracingObject {
  constructor(device, canvasFormat, camera, lights, showTexture = true) {
  ...
  async createGeometry() {
    ...
    this._lightSetting = -1;
    this._shadingSetting = -1;
    this.changeLightSetting();
  }

  changeLightSetting(){
    this._lightSetting = (this._lightSetting + 1) % 3;
    console.log("light setting = " + this._lightSetting);
    var light = this._lights[this._lightSetting];
    let offset = 0;
  ...
  changeShaderSetting(){
    this._shadingSetting = (this._shadingSetting + 1) % 3;
    console.log("shading settting = " + this._shadingSetting);
    for (var i = 0; i < this._lights.length; i++){
      var light = this._lights[i];
      light._params[3] = this._shadingSetting;
    }
    var light = this._lights[this._lightSetting];
    let offset = 0;
    this._device.queue.writeBuffer(this._lightBuffer, offset, light._intensity);
